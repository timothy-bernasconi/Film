...
const filmsRomance = [
    ...
    { genre: "Eau de rose", titre: "À tous les garçons : P.S. Je t’aime toujours" },
...
const filmsComedie = [
    ...
    { genre: "Comédie française", titre: "Astérix et Obélix : Mission Cléopâtre" },
    ...
    { genre: "Comédie américaine", titre: "Anchorman: The Legend of Ron Burgundy" },
...
function choisirFilm(sousGenre, tableauFilms, containerVisible) {
    const filmsFiltrés = tableauFilms.filter(film => film.genre === sousGenre);
    const randomIndex = Math.floor(Math.random() * filmsFiltrés.length);
    const filmChoisi = filmsFiltrés[randomIndex];


    document.querySelector(containerVisible).classList.remove("visible");


    document.getElementById("title").innerHTML = "On vous propose : " + filmChoisi.titre + ".";
}
